...
class EntireCube():
    ...
    def mainloop(self, cubeletmap):
        ...
        animate, animate_ang, animate_speed = False, 0, 40
        action = (0, 0, 0)
        solve = 0
        x = 0
        shot = 0
        ...
        valuex = 350
        valuey = 350
        bufarray = np.zeros((300,valuex,valuey,4))
        rotationconstant = 0
        rotations = 1
        f = open("123.txt", "r")
        if f.mode == 'r':
        ...
        while stopvid == 0:
            ...
            buf = glReadPixels(100, 100, valuex, valuey, GL_RGBA, GL_BYTE)

            bufarray[shot] = buf
            print(shot)

            img = Image.frombuffer('RGBA', (valuex, valuey), buf)
            img.save("temp.png")
            temp = cv2.imread("temp.png")

            cv2.imwrite((r'C:\Users\polog\OneDrive\Documents\RubixMain\Rubix\Rubix\Photos\real\shot%d.jpg' % shot), temp)
...
def main(cubeletmap):
    ...
    bufarray = NewEntireCube.mainloop(cubeletmap)
    return (bufarray)
