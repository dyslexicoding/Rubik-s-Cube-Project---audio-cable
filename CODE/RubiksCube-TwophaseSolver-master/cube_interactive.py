...
class InteractiveCube(plt.Axes):
    ...
    def _initialize_widgets(self):
        self._ax_reset = self.figure.add_axes([0.75, 0.1, 0.2, 0.075])
        self._btn_reset = widgets.Button(self._ax_reset, 'Reset View')
        self._btn_reset.on_clicked(self._reset_view)

        self._ax_solve = self.figure.add_axes([0.55, 0.1, 0.2, 0.075])
        self._btn_solve = widgets.Button(self._ax_solve, 'Solve Cube')
        self._btn_solve.on_clicked(self._solve_cube)

        self._ax_mix = self.figure.add_axes([0.55, 0.00, 0.2, 0.075])
        self._btn_mix = widgets.Button(self._ax_mix, 'Mix Up')
        self._btn_mix.on_clicked(self._mix_cube)
    ...
    def _mix_cube(self, *args):
        move_list, motor_list = mixcube.run(1, 0)
        x = 0
        for (face, n, layer) in move_list[::-1]:
            x = x+ 1
            audio.run(motor_list, x, 0)
            self.rotate_face(face, -n, layer, steps=3)
