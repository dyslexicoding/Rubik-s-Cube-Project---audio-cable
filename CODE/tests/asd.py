...
class EntireCube():
    ...
    def mainloop(self, cubeletmap):
        ...
        animate, animate_ang, animate_speed = False, 0, 2
        action = (0, 0, 0)
        solve = 0
        x = 0
        rotationconstant = 0
        ...
        while True:
            ...
            print("buf")
            buf = glReadPixels(0, 0, 640, 640, GL_RGB, GL_BYTE)
            print(buf)
            #img = Image.frombuffer('RGB', (640, 640), buf)
            #img2 = ImageOps.flip(img)
            #img2.save('frame%06i.png' % (frame_number))
